...
*/
	...
	function getProjectsList()
	{
		...
		$projects=$this->_db->select('select id,name,description,servers_count,modules_count,
			(select sum(size) from jails where project_id=p.id) as size,
			(select count(*) from jails where project_id=p.id) as jails_count
			from projects p');
		
		/*
		$projects=$this->_db->select('select id,name,description,servers_count,sum(j.size) as size1,count(*) as jails_count1
			from projects p, jails j where project_id=p.id group by size1,jails_count1');
		*/
	...
	function getJailsList($clonos=false)
	{
		...
		if(!empty($jails))
		{
			foreach($jails as $key=>$j)
			{
				...
				{
					$query="select * from jails where jname='jail{$id}'";	//ip4_addr
					//echo $query;
